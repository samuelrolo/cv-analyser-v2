...
function transformGeminiResponse(analysis: any): any {
  ...
  try {
    ...
    const automationRisk = { percentage: autoPercentage, level: autoLevel, description: autoDesc, recommendations: autoRecs };

    // Improvement actions with before/after
    const improvementActions = quadrants
    ...
    // Priority matrix
    const priorityMatrix = quadrants.map(q => {
  ...
  return { atsRejectionRate, atsTopFactor, quadrants, keywords, perceivedRole, perceivedSeniority, overallScore: overallScoreNum, salaryDetailed, automationRisk, improvementActions, priorityMatrix, detailedAtsAnalysis, recruiterDeepAnalysis, actionPlan30Days };
}
